...
public class BlockFluid {
    private IBoundedDataBlockCollection blockset;

    private readonly int xdim;
    private readonly int ydim;
    private readonly int zdim;

    private Dictionary<ChunkKey, IBoundedDataBlockCollection> chunks;
    ...
    internal class BlockCoord {
        ...
        internal BlockCoord(IBoundedDataBlockCollection chunk, int lx, int ly, int lz) {
            this.chunk = chunk;
            this.lx = lx;
            this.ly = ly;
            this.lz = lz;
    ...
    public BlockFluid(IBoundedDataBlockCollection blockset) {
        ...
        this.chunks = new Dictionary<ChunkKey, IBoundedDataBlockCollection>();
    }
        ...
        for(int x = 0; x < xdim; x++) {
            for(int z = 0; z < zdim; z++) {
                for(int y = 0; y < ydim; y++) {
                    ...
                    if(info.ID == BlockInfo.StationaryWater.ID && this.blockset.GetData(x, y, z) == 0) {
    ...
    public void RebuildLava() {
        ...
        for(int x = 0; x < xdim; x++) {
            for(int z = 0; z < zdim; z++) {
                for(int y = 0; y < ydim; y++) {
                    BlockInfo info = this.blockset.GetInfo(x, y, z);
                    if(info.ID == BlockInfo.StationaryLava.ID && this.blockset.GetData(x, y, z) == 0) {
                        buildQueue.Add(new BlockKey(x, y, z));
                    }
                }
            }
    ...
    private BlockCoord TranslateCoord(int x, int y, int z) {
        ...
        int lx = ((x % this.xdim) + this.xdim) % this.xdim;
        int lz = ((z % this.zdim) + this.zdim) % this.zdim;

        return new BlockCoord(chunk, lx, y, lz);
    }

    private IBoundedDataBlockCollection GetChunk(int x, int z) {
        int cx = x / this.xdim + (x >> 31);
        int cz = z / this.zdim + (z >> 31);
