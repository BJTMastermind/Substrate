...
public class BlockLight {
    private struct LightRecord {
        ...
        public LightRecord(int x, int y, int z, int s) {
            this.x = x;
            this.y = y;
            this.z = z;
            this.str = s;
    ...
    private IBoundedLitBlockCollection blockset;

    // Maintains internal state of multi-block relighting algorithms
    private BitArray lightbit;
    private Queue<BlockKey> update;
    ...
    public BlockLight(IBoundedLitBlockCollection blockset) {
        ...
        this.xdim = this.blockset.XDim;
        this.ydim = this.blockset.YDim;
        this.zdim = this.blockset.ZDim;

        this.lightbit = new BitArray(this.blockset.XDim * 3 * this.blockset.ZDim * 3 * this.blockset.YDim);
        this.update = new Queue<BlockKey>();
    }

    public BlockLight(BlockLight bl) {
        this.blockset = bl.blockset;
    ...
    public void UpdateBlockLight(int lx, int ly, int lz) {
        ...
        this.update.Enqueue(primary);

        //BlockInfo info = blockset.GetInfo(lx, ly, lz);
    ...
    public void UpdateHeightMap(int lx, int ly, int lz) {
        BlockInfo info = this.blockset.GetInfo(lx, ly, lz);
        int h = Math.Min(ly + 1, this.ydim - 1);

        int height = this.blockset.GetHeight(lx, lz);
        if(h < height) {
            return;
        }
        ...
        if(h == height && !info.ObscuresLight) {
            for(int i = ly - 1; i >= 0; i--) {
                BlockInfo info2 = this.blockset.GetInfo(lx, i, lz);
                if(info2.ObscuresLight) {
                    this.blockset.SetHeight(lx, lz, Math.Min(i + 1, this.ydim - 1));
                    break;
                }
            }
            UpdateBlockSkyLight(lx, h, lz);
        } else if(h > height && info.ObscuresLight) {
            this.blockset.SetHeight(lx, lz, h);
            UpdateBlockSkyLight(lx, h, lz);
        }
    }
        ...
        for(int x = 0; x < xdim; x++) {
            for(int z = 0; z < zdim; z++) {
                for(int y = 0; y < ydim; y++) {
                    BlockInfo info = this.blockset.GetInfo(x, y, z);
                    if(info.Luminance > 0) {
        ...
        for(int x = 0; x < xdim; x++) {
            for(int z = 0; z < zdim; z++) {
                ...
                for(int y = h + 1; y < ydim; y++) {
                    this.blockset.SetSkyLight(x, y, z, BlockInfo.MAX_LUMINANCE);
                }
            ...
            for(int z = 0; z < zdim; z++) {
                for(int y = ydim - 1; y >= 0; y--) {
                    ...
                    if(info.ObscuresLight) {
                        this.blockset.SetHeight(x, z, Math.Min(y + 1, ydim - 1));
                        break;
                    }
                }
            }
    ...
    private void UpdateBlockLight() {
        ...
        while(this.update.Count > 0) {
            BlockKey k = this.update.Dequeue();
            int index = LightBitmapIndex(k);
            this.lightbit[index] = false;
    ...
    private void SpreadBlockLight(IBoundedLitBlockCollection[,] chunkMap, int lx, int ly, int lz) {
        BlockInfo primary = this.blockset.GetInfo(lx, ly, lz);
        int primaryLight = this.blockset.GetBlockLight(lx, ly, lz);
        int priLum = Math.Max(primary.Luminance - primary.Opacity, 0);

        if(primaryLight < priLum) {
            this.blockset.SetBlockLight(lx, ly, lz, priLum);
        }
    ...
    private void SpreadSkyLight(IBoundedLitBlockCollection[,] chunkMap, int[,] heightMap, int lx, int ly, int lz) {
        BlockInfo primary = this.blockset.GetInfo(lx, ly, lz);
        int primaryLight = this.blockset.GetSkyLight(lx, ly, lz);
        int priLum = Math.Max(BlockInfo.MAX_LUMINANCE - primary.Opacity, 0);

        if(primaryLight < priLum) {
            this.blockset.SetSkyLight(lx, ly, lz, priLum);
        }
    ...
    private int LightBitmapIndex(BlockKey key) {
        int x = key.x + this.xdim;
        int y = key.y;
        int z = key.z + this.zdim;

        int zstride = this.ydim;
        int xstride = this.zdim * 3 * zstride;
    ...
    private void QueueRelight(BlockKey key) {
        ...
        if(!this.lightbit[index]) {
            this.lightbit[index] = true;
            this.update.Enqueue(key);
        }
    }

    private IBoundedLitBlockCollection LocalChunk(int lx, int ly, int lz) {
        if(ly < 0 || ly >= this.ydim) {
            return null;
        }
        ...
        if(lx < 0) {
            ...
            } else if(lz >= this.zdim) {
                return OnResolveNeighbor(-1, 0, 1);
            }
            return OnResolveNeighbor(-1, 0, 0);
        } else if(lx >= this.xdim) {
            if(lz < 0) {
                return OnResolveNeighbor(1, 0, -1);
            } else if(lz >= this.zdim) {
                return OnResolveNeighbor(1, 0, 1);
    ...
    private int NeighborHeight(int x, int z) {
        ...
        x = (x + this.xdim * 2) % this.xdim;
        z = (z + this.zdim * 2) % this.zdim;
    ...
    private void TestBlockLight(IBoundedLitBlockCollection chunk, int x1, int y1, int z1, int x2, int y2, int z2) {
        int light1 = this.blockset.GetBlockLight(x1, y1, z1);
        int light2 = chunk.GetBlockLight(x2, y2, z2);
        int lum1 = this.blockset.GetInfo(x1, y1, z1).Luminance;
    ...
    private void TestSkyLight(IBoundedLitBlockCollection chunk, int x1, int y1, int z1, int x2, int y2, int z2) {
        int light1 = this.blockset.GetSkyLight(x1, y1, z1);
