...
public class ChunkCache {
    ...
    public ChunkCache(int cacheSize) {
        this.cache = new LRUCache<ChunkKey, ChunkRef>(cacheSize);
        this.dirty = new Dictionary<ChunkKey, ChunkRef>();

        this.cache.RemoveCacheValue += EvictionHandler;
    }
    ...
    public bool Insert(ChunkRef chunk) {
        ...
        ChunkRef c;
        if(!this.cache.TryGetValue(key, out c)) {
    ...
    public ChunkRef Fetch(ChunkKey key) {
        ChunkRef c;
        if(this.dirty.TryGetValue(key, out c)) {
            return c;
        }
    ...
    public IEnumerator<ChunkRef> GetDirtyEnumerator() {
        return this.dirty.Values.GetEnumerator();
    }
