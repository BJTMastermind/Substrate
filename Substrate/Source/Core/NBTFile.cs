...
public class NBTFile {
    ...
    public int GetModifiedTime() {
        return Timestamp(File.GetLastWriteTime(this.filename));
    }
        ...
        try {
            switch(compression) {
                case CompressionType.None:
                    using(FileStream fstr = new FileStream(this.filename, FileMode.Open, FileAccess.Read, FileShare.ReadWrite)) {
                ...
                case CompressionType.GZip:
                    Stream stream1 = new FileStream(this.filename, FileMode.Open, FileAccess.Read, FileShare.ReadWrite);
                    return new GZipStream(stream1, CompressionMode.Decompress);
                case CompressionType.Zlib:
                    Stream stream2 = new FileStream(this.filename, FileMode.Open, FileAccess.Read, FileShare.ReadWrite);
                    return new ZlibStream(stream2, CompressionMode.Decompress);
                case CompressionType.Deflate:
                    Stream stream3 = new FileStream(this.filename, FileMode.Open, FileAccess.Read, FileShare.ReadWrite);
    ...
    class NBTBuffer : MemoryStream {
        ...
        public NBTBuffer(NBTFile c) : base(8096) {
            this.file = c;
        }

        public override void Close() {
            try {
                using(Stream fstr = new FileStream(this.file.filename, FileMode.Create, FileAccess.Write, FileShare.ReadWrite)) {
                    try {
