...
public class MapConverter {
    private static Color[] defaultColorIndex;

    private Color[] colorIndex;
    private ColorGroup[] blockIndex;

    private int groupSize = 4;

    private Vector3[] labIndex;

    /// <summary>
    /// Creates a new <see cref="MapConverter"/> with a Minecraft-default color-index and block-index.
    /// </summary>
    public MapConverter() {
        this.colorIndex = new Color[256];
        this.labIndex = new Vector3[256];
        defaultColorIndex.CopyTo(this.colorIndex, 0);

        RefreshColorCache();

        // Setup default block index
        this.blockIndex = new ColorGroup[4096];
        for(int i = 0; i < this.blockIndex.Length; i++) {
            this.blockIndex[i] = ColorGroup.Other;
        }

        this.blockIndex[BlockInfo.Grass.ID] = ColorGroup.Grass;
        this.blockIndex[BlockInfo.TallGrass.ID] = ColorGroup.Grass;
        this.blockIndex[BlockInfo.Sand.ID] = ColorGroup.Sand;
        this.blockIndex[BlockInfo.Gravel.ID] = ColorGroup.Sand;
        this.blockIndex[BlockInfo.Sandstone.ID] = ColorGroup.Sand;
        this.blockIndex[BlockInfo.Lava.ID] = ColorGroup.Lava;
        this.blockIndex[BlockInfo.StationaryLava.ID] = ColorGroup.Lava;
        this.blockIndex[BlockInfo.Ice.ID] = ColorGroup.Ice;
        this.blockIndex[BlockInfo.Leaves.ID] = ColorGroup.Leaves;
        this.blockIndex[BlockInfo.YellowFlower.ID] = ColorGroup.Leaves;
        this.blockIndex[BlockInfo.RedRose.ID] = ColorGroup.Leaves;
        this.blockIndex[BlockInfo.Snow.ID] = ColorGroup.Snow;
        this.blockIndex[BlockInfo.SnowBlock.ID] = ColorGroup.Snow;
        this.blockIndex[BlockInfo.ClayBlock.ID] = ColorGroup.Clay;
        this.blockIndex[BlockInfo.Dirt.ID] = ColorGroup.Dirt;
        this.blockIndex[BlockInfo.Stone.ID] = ColorGroup.Stone;
        this.blockIndex[BlockInfo.Cobblestone.ID] = ColorGroup.Stone;
        this.blockIndex[BlockInfo.CoalOre.ID] = ColorGroup.Stone;
        this.blockIndex[BlockInfo.IronOre.ID] = ColorGroup.Stone;
        this.blockIndex[BlockInfo.GoldOre.ID] = ColorGroup.Stone;
        this.blockIndex[BlockInfo.DiamondOre.ID] = ColorGroup.Stone;
        this.blockIndex[BlockInfo.RedstoneOre.ID] = ColorGroup.Stone;
        this.blockIndex[BlockInfo.LapisOre.ID] = ColorGroup.Stone;
        this.blockIndex[BlockInfo.Water.ID] = ColorGroup.Water;
        this.blockIndex[BlockInfo.StationaryWater.ID] = ColorGroup.Water;
        this.blockIndex[BlockInfo.Wood.ID] = ColorGroup.Wood;
    }
    ...
    public int BlockToColorIndex(int blockId, int level) {
        if(level < 0 || level >= this.groupSize) {
            throw new ArgumentOutOfRangeException("level", level, "Argument 'level' must be in range [0, " + (this.groupSize - 1) + "]");
        }
        if(blockId < 0 || blockId >= 4096) {
            throw new ArgumentOutOfRangeException("blockId");
        }

        return (int) this.blockIndex[blockId] * this.groupSize + level;
    }
    ...
    public ColorGroup ColorIndexToGroup(int colorIndex) {
        int group = colorIndex / this.groupSize;
    ...
    public int GroupToColorIndex(ColorGroup group, int level) {
        ...
        return (int) group * this.groupSize + level;
    }
    ...
    public void RefreshColorCache() {
        for(int i = 0; i < this.colorIndex.Length; i++) {
            this.labIndex[i] = RgbToLab(this.colorIndex[i]);
        }
    }
    ...
    public int NearestColorIndex(Color color) {
        ...
        for(int i = 0; i < this.colorIndex.Length; i++) {
            if(this.colorIndex[i].A == 0) {
                continue;
            }

            double x = cr.X - this.labIndex[i].X;
            double y = cr.Y - this.labIndex[i].Y;
            double z = cr.Z - this.labIndex[i].Z;
    ...
    public Color NearestColor(Color color) {
        return this.colorIndex[NearestColorIndex(color)];
    }
    ...
    public Bitmap MapToBitmap(Map map) {
        ...
        for(int x = 0; x < map.Width; x++) {
            for(int z = 0; z < map.Height; z++) {
                Color c = this.colorIndex[map[x, z]];
                bmp.SetPixel(x, z, c);
            }
        }
        return bmp;
