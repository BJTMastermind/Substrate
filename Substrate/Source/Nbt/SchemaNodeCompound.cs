...
public sealed class SchemaNodeCompound : SchemaNode, ICollection<SchemaNode> {
    ...
    public bool Contains(SchemaNode item) {
        return this.subnodes.Contains(item);
    }
    ...
    public void CopyTo(SchemaNode[] array, int arrayIndex) {
        this.subnodes.CopyTo(array, arrayIndex);
    }
    ...
    /// </summary>
    public int Count {
        get { return this.subnodes.Count; }
    }
    ...
    public bool Remove(SchemaNode item) {
        return this.subnodes.Remove(item);
    }
    ...
    public IEnumerator<SchemaNode> GetEnumerator() {
        return this.subnodes.GetEnumerator();
    }
    ...
    public SchemaNodeCompound()
        : base("") {
        this.subnodes = new List<SchemaNode>();
    }
    ...
    public SchemaNodeCompound(string name, SchemaNode subschema)
        ...
        foreach(SchemaNode node in schema.subnodes) {
    ...
    public SchemaNodeCompound MergeInto(SchemaNodeCompound tree) {
        foreach(SchemaNode node in this.subnodes) {
            SchemaNode f = tree.subnodes.Find(n => n.Name == node.Name);
            if(f != null) {
                continue;
            }
            tree.Add(node);
